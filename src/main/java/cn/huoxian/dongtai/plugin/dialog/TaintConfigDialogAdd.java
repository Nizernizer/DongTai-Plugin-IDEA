...
public class TaintConfigDialogAdd extends JDialog {
    ...
    public TaintConfigDialogAdd(String methodSignature, String classKind, Map<String, Integer> map, Project project) {
        ...
        ruleSetComboBox.addActionListener(e -> {
            ...
            if (TaintConstant.RULE_STAIN_SOURCE.equals(selectedItem)) {
                setVisible(false);
                TaintConfigDialog dialog = new TaintConfigDialog(methodSignature, classKind,project);
                dialog.setRuleSet(1);
                dialog.pack();
    ...
    private void requestJson(String url,Project project) {
        ...
        try {
            ...
            if (TaintConstant.REQUEST_JSON_ERROR_STATUS.equals(parameter)) {
                notificationError(project,TaintConstant.NOTIFICATION_CONTENT_ERROR_COMPLETE);
            }
        } catch (IOException e1) {
            String content = "请求未发送成功，请检查 DongTai IAST自定义规则 中的配置是否正确！";
            notificationError(project,content);
        }
    }
}
