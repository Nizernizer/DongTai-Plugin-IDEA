...
public class TaintConfigDialog extends JDialog {
    ...
    public TaintConfigDialog(String methodSignature, String classKind, Project project) {
        ...
        if (taintSourceMethodRules == null || "".equals(taintSourceMethodRules)){
            notificationError(project,TaintConstant.NOTIFICATION_CONTENT_ERROR_CONNECT);
        }
        ...
        ruleSetComboBox.addActionListener(e -> {
            try {
                ...
                if (TaintConstant.RULE_STAIN_SOURCE.equals(selectedItem)) {
                    ...
                    TaintConfigDialogAdd dialog = new TaintConfigDialogAdd(methodSignature, classKind, dangerousMethodRulesMap,project);
                    dialog.pack();
                    dialog.setVisible(true);
                }
            }catch (Exception ignore){
                notificationError(project,TaintConstant.NOTIFICATION_CONTENT_ERROR_RULE);
        ...
        buttonOk.addActionListener(e -> {
            ...
            requestJson(url + TaintConstant.RULESET_API_RULE_ADD,project);
            onOk();
        });
    ...
    private void requestJson(String url,Project project) {
        ...
        try {
            ...
            notificationInfo(project,TaintConstant.NOTIFICATION_CONTENT_INFO_SUCCESS);
        } catch (IOException e1) {
            notificationError(project,TaintConstant.NOTIFICATION_CONTENT_ERROR_FAILURE);
        }
    }
    ...
    public Map<String, Integer> setRuleSource(String json) {
        ...
        for (int i = 0; i < data.length(); i++) {
            JSONObject jsonElement = (JSONObject) data.get(i);
            Integer id = (Integer) jsonElement.get("id");
            String name = (String) jsonElement.get("name");
            ruleSourceMap.put(name, id);
        }
